...
def make_graph(xyz, pair, idx, top_k=18, kmin=12):
    ...
    avg_distances = D.mean(dim=-1)  # (B, L)
    max_avg_dist = avg_distances.max()
    min_avg_dist = avg_distances.min()

    # dynamically adjust top_k based on average distances
    adaptive_top_k = ((avg_distances - min_avg_dist) / (max_avg_dist - min_avg_dist + 1e-9) * (top_k - kmin)) + kmin
    adaptive_top_k = adaptive_top_k.long()

    # create topk_matrix in one go
    _, E_idx = torch.topk(D, min(top_k, L), largest=False) # get top_k indices for all b,l
    mask = torch.arange(top_k, device=device)[None, None, :] < adaptive_top_k[:, :, None] # create mask using adaptive_top_k
    topk_matrix = torch.zeros((B, L, L), device=device)
    topk_matrix.scatter_(2, E_idx, mask.float())  # fill topk_matrix using the mask
...
#         self.lossfn = nn.MSELoss(reduction='sum')
    ...
    def training_step(self, batch, batch_idx):
        ...
        nodes, pair, bond, init_pos, init_xyz, init_atom, init_CA, label, seq_l, pdbs, features = batch
        bsz = 1
        L = seq_l[0]
        print("batch L: ", L)
