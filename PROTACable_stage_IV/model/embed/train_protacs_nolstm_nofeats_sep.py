...
from se3_transformer_protacs_nolstm_nofeats_sep import *
import warnings
...
def _collate_fn(batch):
    ...
    return nodes, pairs, bonds, init_xyz, init_pos, init_atom, init_CA, label_xyz, seq_l, pdbs, features, sep #, res_atom
...
def run_script(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='val_acc',
        dirpath=out_path,
        filename=network + '-{epoch:02d}-{val_auc_roc:.3f}-{val_acc:.3f}-{val_f1:.3f}-{val_loss:.3f}',
        save_top_k=100,
        mode='max',
        every_n_epochs=1,
        save_last=True
    ...
    if debug_mode:
        ...
        trainer = pl.Trainer(max_epochs=epochs, check_val_every_n_epoch=1,
                             accumulate_grad_batches=batch_size*16,
                             default_root_dir=out_path,
                             accelerator='cuda',
