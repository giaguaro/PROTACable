...
class ElabMols():
 ...
 #       att_94 = neigh94 + mol1.GetNumAtoms()
    ...
    def align(self, protein, mol1, mol2, mol3, du1, du2, ca1, ca2, ca3, radiolabels):
        ...
        distance = np.linalg.norm(np.array(mol1.GetConformer().GetAtomPosition(ca1)) - np.array(mol3.GetConformer().GetAtomPosition(ca3)))

        if distance > 15:
            return final_molecule, 999
        ...
        #    aligned_frag_to_mol1_conformers = self.align(mol1, frag, mol1_indices[1], frag_indices, mol1_indices[0], frag_indices)
            ...
            if best_distance > 5.0:
                ...
                if best_distance == 999:
                    return final_molecule, True
                else:
                    return final_molecule, False
