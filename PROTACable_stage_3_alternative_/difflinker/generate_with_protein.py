...
def read_molecule(path):
    if path.endswith('.pdb'):
        return (Chem.MolFromPDBFile(path), Chem.MolFromPDBFile(path, sanitize=False, removeHs=True))
...
def main(input_path, protein_path, backbone_atoms_only, model,
    ...
    try:
        HMolecule, molecule = read_molecule(input_path)
    ...
    frag_pos, frag_one_hot, frag_charges, anchors = parse_molecule(HMolecule, molecule, is_geom=ddpm.is_geom)

    print(f"detected anchor sites {anchors[0]} and {anchors[1]}")
    ...
    if anchors is not None:
        for anchor in [29, 82]: #anchors:
            anchor_flags[anchor] = 1
