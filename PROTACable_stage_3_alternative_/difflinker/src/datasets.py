...
def parse_molecule(h_mol, mol, is_geom):
    one_hot = []
    charges = []
    ...
    for atom in h_mol.GetAtoms():
        ...
        if residue_info is not None:
            residue_name = residue_info.GetName().strip()
            if '90' in residue_name or '95' in residue_name or '96' in residue_name or '97' in residue_name:
                anchors.append(atom.GetIdx())

    if len(anchors) < 2:
        raise Exception('Did you forget to label your anchor as *90 (where * is an atom symbol) before starting this program?!')

    positions = mol.GetConformer().GetPositions()
    return positions, np.array(one_hot), np.array(charges), tuple(anchors)
